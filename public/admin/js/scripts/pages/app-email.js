...
$(function () {
	...
	var appContent = $(".app-content"),
	  ...
	  userNewMailSideBar = $(".compose-new-email-sidebar"),
	...
	if (!$.app.menu.is_touch_device()) {
	  ...
	  if (userNewMailSideBar.length > 0) {
		new PerfectScrollbar(".compose-new-email-sidebar", {
		  wheelPropagation: false
		});
	  }
	} else {
	  ...
	  $('.compose-new-email-sidebar').css('overflow', 'scroll')
	}
	// --------------------------------------
	// Quill Editor
	//---------------------------------------

	//  Details email Message reply field
	...
	// Sidebar menu close button on click remove show class form both compose email sidebar and App content overlay
	...
	email_application.find(".email-delete").on("click", function () {
	...
	// Mark unread email and remove background color when checkbox unchecked
	email_application.find(".email-unread").on("click", function () {
	  checkbox_con.find("input:checked").closest("li").removeClass("email-read");
	...
	// ------------------------------------------
	// Compose new email sidebar
	// -------------------------------------------

	// On Click of Close Icon btn, cancel btn and overlay remove show class from compose email and overlay
	...
	// On compose btn clik of compose email visible and sidebar left hide
